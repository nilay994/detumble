...
Example that uses the UART driver to echo back to the console. Using Hterm to mock the HIL tests currently

- changed the rxBuffer sizes, now waits for newline
- some uncertainty around null terminated strings and string lengths
- malloc for char buffer and memset to null terminate after parsing
- strtok for csv parsing added
- FPU is now enabled
